...
test.describe('피드백 입력 시나리오', () => {
  ...
  test('[TC-047] AI 문체 변환 OFF로 피드백을 작성하고 제출하면 성공 메시지가 표시된다', async ({ page }) => {
    ...
    // 피드백 입력 (pressSequentially로 Vue v-model 트리거)
    const textarea = page.locator('[data-testid="feedback-textarea"]')
    await textarea.click()
    await textarea.pressSequentially('배포 프로세스가 너무 오래 걸립니다.')
    ...
    // 제출 버튼 활성화 대기 후 클릭
    await expect(page.locator('[data-testid="submit-button"]')).toBeEnabled({ timeout: 3000 })
  ...
  test('[TC-048] AI 문체 변환 ON 상태에서 피드백 제출 시 변환 미리보기 모달이 표시된다', async ({ page }) => {
    ...
    await page.click('[data-testid="category-tab-problem"]')
    const textarea48 = page.locator('[data-testid="feedback-textarea"]')
    await textarea48.click()
    await textarea48.pressSequentially('배포할 때마다 30분씩 걸리는 거 진짜 미치겠음... ㅠㅠ')

    // AI 변환 토글 ON (sr-only 체크박스는 force 옵션 필요)
    await page.locator('[data-testid="ai-transform-toggle"]').check({ force: true })
